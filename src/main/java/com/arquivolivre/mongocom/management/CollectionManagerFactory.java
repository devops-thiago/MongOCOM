...
public final class CollectionManagerFactory {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(CollectionManagerFactory.class);
     ...
     *
     * @param uri MongoDB's connection URI (e.g., "mongodb://user:password@host:port/database")
     * @return an instance of a <code>CollectionManager</code>.
     */

    public static CollectionManager createCollectionManagerFromUri(String uri) {
        try {
            LOG.info("Connected to MongoDB using URI: {}", uri);
            return new CollectionManager(client, getConn(uri).getDatabase());
        } catch (MongoException | IllegalArgumentException ex) {
            LOG.error("Unable to connect to MongoDB using URI: {}, error: ", uri, ex);
        }
        return null;
    }
    ...
    private static CollectionManager createBaseCollectionManager(
        ...
        try {
            ...
            LOG.error("Unable to connect to a mongoDB instance, maybe it is not running "
                    + "or you do not have the right permission: ", ex);
        }
        return null;
    }
    ...
    public static CollectionManager setup(ServletContext context) {
        try {
            ...
            final String user = properties.containsKey("mongocom.user")
                    ? properties.getProperty("mongocom.user") : "";
            final String password =
                    properties.containsKey("mongocom.password")
                            ? properties.getProperty("mongocom.password")
                            : "";
            final String host = properties.containsKey("mongocom.host")
                    ? properties.getProperty("mongocom.host") : "";
            final String port = properties.containsKey("mongocom.port")
                    ? properties.getProperty("mongocom.port") : "";
            final String dbName =
            ...
            LOG.info("Constructed MongoDB URI from individual properties: {}",
                    builder);
            ...
            LOG.error("Error setting up CollectionManager", ex);
        }
        return null;
    }
