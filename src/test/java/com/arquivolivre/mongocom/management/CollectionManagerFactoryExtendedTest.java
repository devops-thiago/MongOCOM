...
class CollectionManagerFactoryExtendedTest {
    ...
    void testCreateCollectionManagerWithHostAndPort() {
        assertDoesNotThrow(() -> {
            CollectionManager cm = CollectionManagerFactory.createCollectionManager("localhost", 27017);
            // May be null if connection fails, which is expected in test environment
        });
    }
    ...
    void testCreateCollectionManagerWithCredentials() {
        assertDoesNotThrow(() -> {
            CollectionManager cm = CollectionManagerFactory.createCollectionManager("testdb", "user", "password");
            // May be null if connection fails, which is expected in test environment
        });
    }
    ...
    void testBaseCollectionManagerEmptyParameters() {
        ...
        assertDoesNotThrow(() -> {
            ...
            CollectionManager cm = CollectionManagerFactory.createCollectionManager("", 0, "", "", "");
            // May be null if connection fails, which is expected in test environment
        });
    }
